...
class VanillaNeZhaForSequenceClassification(NeZhaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    Examples::
        ...
        if labels is not None:
            for i, drop_out in enumerate(self.multi_dropout):
                logits = self.classifier(drop_out(pooled_output))
                if i == 0:
...
class NeZhaForSequenceClassification(NeZhaPreTrainedModel):
    def __init__(self, config):
        ...
        self.pooler = BertPooler(config)
        self.multi_dropout = nn.ModuleList([nn.Dropout(config.hidden_dropout_prob) for _ in range(config.multi_dropout)])
    ...
    Examples::
        ...
        # hidden_pool1 = self.dropout(self.pooler(outputs[2][-1]))
        # hidden_pool2 = self.dropout(self.pooler(outputs[2][-2]))
        # hidden_pool3 = self.dropout(self.pooler(outputs[2][-3]))
        # hidden_pool4 = self.dropout(self.pooler(outputs[2][-4]))
        #
        # hidden_mean_pool1 = torch.mean(outputs[2][-1], dim=1)
        # # hidden_max_pool1, _ = torch.max(outputs[2][-1], dim=1)
        #
        # pooled_output = outputs[1]
        # pooled_output = self.dropout(pooled_output)
        # logits = self.classifier(torch.cat([pooled_output, hidden_pool1, hidden_pool2, hidden_pool3, hidden_pool4,
        #                                     hidden_mean_pool1], dim=-1))

        hidden_pool1 = self.pooler(outputs[2][-1])
        # hidden_pool2 = self.pooler(outputs[2][-2])
        # hidden_pool3 = self.pooler(outputs[2][-3])

        hidden_mean_pool1 = torch.mean(outputs[2][-1], dim=1)
        # hidden_mean_pool2 = torch.mean(outputs[2][-2], dim=1)
        # hidden_mean_pool3 = torch.mean(outputs[2][-3], dim=1)
        # hidden_mean_pool4 = torch.mean(outputs[2][-4], dim=1)

        hidden_max_pool1, _ = torch.max(outputs[2][-1], dim=1)
        # hidden_max_pool2, _ = torch.max(outputs[2][-2], dim=1)
        ...
        if labels is not None:
            for i, dropout in enumerate(self.multi_dropout) :
                logits = self.classifier(
                    dropout(
                        torch.cat([pooled_output, hidden_pool1, hidden_mean_pool1, hidden_max_pool1,], dim=-1)))
                if i == 0:
        ...
        return output  # (loss), logits, (hidden_states), (attentions)
...
class DoubleHeadNeZhaForSequenceClassification(NeZhaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = NeZhaModel(config)
        ...
        # self.multi_dropout2 = nn.ModuleList(
        #     [nn.Dropout(config.hidden_dropout_prob) for _ in range(config.multi_dropout)])
    ...
    Returns:
        ...
        # ---------------------------------------------
        # attn_pool1 = self.pooler2(outputs[3][-1])
        # attn_pool2 = self.pooler2(outputs[3][-1])
        # attn_pool3 = self.pooler2(outputs[3][-3])
        #
        # attn_mean_pool1 = torch.mean(outputs[3][-1][:, 1:, :], dim=1)
        # attn_mean_pool2 = torch.mean(outputs[3][-2][:, 1:, :], dim=1)
        # # hidden_mean_pool3 = torch.mean(outputs[2][-3], dim=1)
        # # hidden_mean_pool4 = torch.mean(outputs[2][-4], dim=1)
        #
        # attn_max_pool1, _ = torch.max(outputs[3][-1][:, 1:, :], dim=1)
        # attn_max_pool2, _ = torch.max(outputs[3][-2][:, 1:, :], dim=1)
        ...
        if labels is not None:
            for i, dropout in enumerate(self.multi_dropout) :
                logits = self.classifier(
                    dropout(
                        torch.cat([pooled_output, hidden_pool1, hidden_pool2,
                                   hidden_pool3, hidden_mean_pool1,
                                   hidden_mean_pool2, hidden_max_pool1, hidden_max_pool2], dim=-1)))
                logits2 = self.classifier2(dropout(pooled_output))

                if i == 0:
                    loss = self.loss_fct(logits.view(-1, self.num_labels), labels.view(-1)) + \
                        self.loss_fct(logits2.view(-1, self.num_labels), labels.view(-1))
                else:
                    loss = loss + self.loss_fct(logits.view(-1, self.num_labels), labels.view(-1)) + \
                           self.loss_fct(logits2.view(-1, self.num_labels), labels.view(-1))
            output = (loss,) + ((logits + logits2)/2, )
        else:
            logits = self.classifier(
                    torch.cat([pooled_output, hidden_pool1, hidden_pool2,
                               hidden_pool3, hidden_mean_pool1,
